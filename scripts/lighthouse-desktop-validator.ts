...
function loadReport(filePath: string): AssertionReport | [] {
  if (!fs.existsSync(filePath)) {
    throw new Error(`❌ Desktop: no such file assertion-results.json (${filePath})`);
  }
...
function processReport(report: AssertionReport | []): void {
  ...
  if (Array.isArray(report) && report.length === 0) {
    console.log('✅ Desktop: ALL TESTS ARE PASSED');
    process.exit(0);
  }
  ...
  if (!Array.isArray(report)) {
    ...
    if (summary?.numFailing && summary.numFailing > 0) {
      console.error('❌ Desktop: TESTS ARE FAILED!');
    ...
    if (summary?.numWarnings && summary.numWarnings > 0) {
      console.warn('⚠️ Desktop: WARNINGS!');
      process.exit(0);
    }
  ...
  throw new Error('❌ Desktop: incorrect format of assertion-results.json');
}

function main(): void {
  try {
    const reportsDir = '.lighthouseci';
    const reportFile = 'desktop-assertion-results.json';
