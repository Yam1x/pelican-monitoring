...
(async () => {
  ...
  try {
    ...
    console.log(`📊 Отчет метрик для Home Page:`);
    ...
    if (max >= 5000) throw new Error(`❌ Максимальное время ответа слишком высокое: ${max}`);
    if (avg >= 1500) throw new Error(`❌ Среднее время ответа слишком высокое: ${avg}`);
